...
#ifndef MAGICMIND_UTILS_HALF_H_
#define MAGICMIND_UTILS_HALF_H_
...
// TODO(hexi): open this macro will cause compile error
...
inline half fma(half x, half y, half z) {
  ...
  if (absx >= 0x7C00 || absy >= 0x7C00 || absz >= 0x7C00)
    return (absx > 0x7C00 || absy > 0x7C00 || absz > 0x7C00)
               ...
               : (absx == 0x7C00)
                     ? half(detail::binary, (!absy || (sub && absz == 0x7C00)) ? detail::invalid()
                                                                               : (sign | 0x7C00))
...
inline half erfc(half arg) {
  ...
  if (!abs)
    return half(detail::binary, 0x3C00);
  if (abs >= 0x4400)
    return half(detail::binary, detail::rounded<half::round_style, true>((sign >> 1) - (sign >> 15),
                                                                         sign >> 15, 1));
